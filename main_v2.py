...
class ProjectSpiders(scrapy.Spider):
   name = "main_two"
          ...
          for j in selectors:
             def parse(self, response):
                if response.status == 200:
                   facility_name = hxs.select('//span[contains(@id, "ContentPlaceHolder1_gvFacilitySearchDetail_FacilityName_0")]/          text()').extract()

	           administrator = hxs.select('//span[contains(@id, "ContentPlaceHolder1_gvFacilitySearchDetail_Label2_0")]/text()').extract()

                   address       = hxs.select('//span[contains(@id, "ContentPlaceHolder1_gvFacilitySearchDetail_lblAddress_0")]/text()').extract()

                   phone_number  = hxs.select('//span[contains(@id, "ContentPlaceHolder1_gvFacilitySearchDetail_lblPhoneNumber_0")]/text()').extract()

                   operator      = hxs.select('//span[contains(@id, "ContentPlaceHolder1_gvFacilitySearchDetail_lbloperator_0")]/text()').extract()

                   total_number_licensed_beds = hxs.select('//span[contains(@id,  "ContentPlaceHolder1_gvFacilitySearchDetail_lblTotalLicensedBeds_0")]/text()').extract()

                   medicare_beds = hxs.select('//span[contains(@id, "ContentPlaceHolder1_gvFacilitySearchDetail_lblMedicareBeds_0")]/text()').extract()

                   medicaid_beds = hxs.select('//span[contains(@id, "ContentPlaceHolder1_gvFacilitySearchDetail_lblMedicaidBeds_0")]/text()').extract()
